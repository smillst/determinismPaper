...
In addition to the type qualifiers described in~\ref{type-hierarchy}, \theDeterminismChecker 
...
\begin{figure}
    ...
    $\infer[\textsc{ordernondet collection}]{\wellformed{\kappa_c \  \beta_c \langle\kappa_e \ \beta_e\rangle}}{\kappa_c : \|OrderNonDet/Det| & \kappa_e \sqsubseteq \kappa_c & \beta_c \sqsubseteq \|Collection/Iterator|}$
    
    \bigskip
    
    $\infer[\textsc{nondet collection}]{\wellformed{\|NonDet| \ \beta_c\langle\|NonDet| \ \beta_e\rangle}}{\beta_c \sqsubseteq \|Collection/Iterator|}$
...
%    \bigskip
    ...
    $\infer[\textsc{ordernondet map}]{\wellformed{\kappa_m \  \beta_m \langle\kappa_k \ \beta_k, \kappa_v \ \beta_v\rangle}}{\kappa_m: \|OrderNonDet/Det| & \kappa_k \sqsubseteq \kappa_m & \kappa_v \sqsubseteq \kappa_m & \beta_m \sqsubseteq \|Map|}$
    
    \bigskip
    
    $\infer[\textsc{nondet map}]{\wellformed{\|NonDet| \ \beta_m\langle\|NonDet| \ \beta_k, \|NonDet| \ \beta_v\rangle}}{\beta_m \sqsubseteq \|Map|}$
...
Recall that \theDeterminismChecker does not permit \<OrderNonDet> on a type that isn't a \<Collection>, \<Iterator>, or an array.
...
\begin{figure}
    $\infer[\textsc{poly resolution}]{\pi_r \ \beta_r method(\overline{\pi_i \ \beta_i})\{\ \}}{\forall K \in \{\|NonDet, OrderNonDet, Det|\} : K \ \beta_r method(\overline{K \ \beta_i})\{\ \} & \pi_r, \pi_i \in \{\|PolyDet|, \|PolyDet|\uparrow, \|PolyDet|\downarrow    \}}$
...
\TheDeterminismChecker has additional type refinement rules for the sorting methods as shown in Figure~\ref{fig-sorting}. 
\begin{figure}
    $\infer[\textsc{list sort}]{\Gamma \vdash x : \|Det \ List|\langle\|Det| \ \beta\rangle}{\Gamma \vdash x : \|OrderNonDet \ List|\langle\|Det| \ \beta\rangle & \|x.sort()|}$
    
    \bigskip
    
    $\infer[\textsc{collections sort}]{\Gamma \vdash x : \|Det \ List|\langle\|Det| \ \beta\rangle}{\Gamma \vdash x : \|OrderNonDet \ List|\langle\|Det| \ \beta\rangle & \|Collections.sort.sort(x)|}$
...
type argument of this \codeid{List} have the type qualifier \codeid{Det} (Rule LIST SORT in Figure~\ref{fig-sorting}). Similar rules apply for \codeid{Arrays.sort()} and \codeid{Collections.sort()}.
...
\begin{figure}
    \todo{linebreak}
    $\infer[\textsc{set precision}]{\Gamma \vdash \|z : Det|}{\Gamma \vdash \|x| : \|OrderNonDet| \ \|Set|\langle \kappa_{x} \ \beta_{x}\rangle, \|y| : \|OrderNonDet| \ \|Set|\langle \kappa_{y} \ \beta_{y}\rangle & \|z = x.equals(y)| & \langle \kappa_{x} \ \beta_{x}\rangle \nsqsubseteq \langle \|OrderNonDet \ List| \rangle & \langle \kappa_{y} \ \beta_{y}\rangle \nsqsubseteq \langle \|OrderNonDet \ List| \rangle }$
    
    \bigskip
    
    $\infer[\textsc{map precision}]{\Gamma \vdash \|z : Det|}{\Gamma \vdash \|x : OrderNonDet \ Map\langle Det \ \beta_k, \kappa_v\ \beta_v \rangle, y : Det \ \beta| & \|z = x.get(y)|}$
...
Similarly, the \codeid{get()} method in the \<Map> class is annotated with polymorphic qualifiers and the rule $\sc{\|MAP \ PRECISION|}$ in Figure~\ref{fig-precision-rules} improves precision for \<OrderNonDet> receivers.
