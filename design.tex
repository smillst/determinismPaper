...
(Rules \rulename{get}, \rulename{iteration}, and \rulename{next element} in \cref{fig-ordernondet-rules}.)
\item
...
This section introduces the another polymorphism feature by contrasting the
...
\noindent
It cannot be instantiated at $\kappa = \<OrderNonDet>$, but the other two
instantiations are enough to ensure that if either the list or the index is (order) non-deterministic, then the result is
nondeterministic.
...
\begin{figure}
    ...
    {
        &\Gamma \vdash \|x| : \<OrderNonDet> \ \|Set|\angles{ \kappa_{x} \ \beta_{x}}, \|y| : \<OrderNonDet> \ \|Set|\angles{ \kappa_{y} \ \beta_{y}} \\
        & \kappa_{x} \ \beta_{x} \nsqsubseteq \<OrderNonDet> \ \<List>        \hspace{150pt}   \\
        & \kappa_{y} \ \beta_{y} \nsqsubseteq \<OrderNonDet> \ \<List>     \hspace{150pt}    
    }
    \end{align*}
    \bigskip
