...
\begin{itemize}
    ...
    \item \<OrderNonDet> indicates that the expression is a collection or
      map (dictionary or associative array) that
      will have the same elements in every execution, but in a possibly different order.
...
\begin{figure}
    ...
    $\infer[\textsc{ordernondet collection}]{\wellformed{\kappa_c \  \beta_c \langle\kappa_e \ \beta_e\rangle}}{\kappa_c : \|OrderNonDet/Det| & \kappa_e \sqsubseteq \kappa_c & \beta_c \sqsubseteq \<Collection>}$
    
    \bigskip
    
    $\infer[\textsc{nondet collection}]{\wellformed{\|NonDet| \ \beta_c\langle\|NonDet| \ \beta_e\rangle}}{\beta_c \sqsubseteq \<Collection>}$
...
\todo{The collection rules do not permit nested collections; similarly, the
  ...
  Otherwise, these rules permit construction of $\<Map>\langle\tau'\rangle$
...
Figure~\ref{type-validity} shows the type validity\todo{The caption says
...
Some examples of valid types are:\todo{double-check}
\begin{itemize}
    \item \codeid{NonDet\ \ \ \ \ \ List<NonDet\ \ \ \ \ \ Integer>}
    \item \codeid{OrderNonDet List<OrderNonDet Set<...>\relax >}
    \item \codeid{OrderNonDet List<Det Integer>}
    \item \codeid{Det\ \ \ \ \ \ \ \ \ List<Det\ \ \ \ \ \ \ \ \ Integer>}
\end{itemize}

These types are invalid:
\begin{itemize}
    \item \codeid{NonDet\ \ \ \ \ \ List<OrderNonDet Integer>}
    \item \codeid{NonDet\ \ \ \ \ \ List<Det\ \ \ \ \ \ \ \ \ Integer>}
    \item \codeid{OrderNonDet List<NonDet\ \ \ \ \ \ Integer>}
    \item \codeid{Det\ \ \ \ \ \ \ \ \ List<NonDet\ \ \ \ \ \ Set<...>\relax >}
...
%     \bigskip
    ...
    $\infer[\textsc{sort}]{\Gamma \vdash x : \|Det \ List|\langle\|Det| \ \beta\rangle}{\Gamma \vdash x : \|OrderNonDet \ List|\langle\|Det| \ \beta\rangle & \|Collections.sort(x)|}$
...
type argument of this \codeid{List} have the type qualifier \codeid{Det} (Rule LIST SORT in Figure~\ref{fig-sorting}).
