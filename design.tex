...
Figure~\cref{typecheck-rules-standard} shows a sample of the standard typing rules applied to 
...
Formally, $\beta ::= C|C \angles{\tau}$ where $C$ represents a valid Java basetype.
\ourTypeSystem checks all the standard typing rules including the ones shown in Figure~\cref{typecheck-rules-standard} with
the only difference of having an additional type qualifier on each of the basetypes. The type qualifiers can only 
constrain the set of valid types. As a result, \ourTypeSystem doesn't allow any behavior that the original type system doesn't.
...
\smallskip
 ...
 Although \<NonDet> collections containing \<Det> or \<OrderNonDet> elements could exist, they are
...
\begin{enumerate}
    \item The individual elements extracted by iterating over the collection get the type \<NonDet> (Rules \rulename{iteration} and \rulename{next element} in Figure~\ref{fig-ordernondet-rules}). 
    Rule \rulename{hasnext element} states that querying if the iterator of an \<OrderNonDet Collection> \<hasNext()> will result in a \<Det boolean>.
