...
In this type system, \<Det> is a subtype of \<NonDet> denoted as \codeid{Det $\sqsubseteq$ NonDet}.
...
%$\infer{\vdash : \kappa \  \beta}{isParam(\beta) / isReturn(\beta) & \kappa : @PolyDet("up") / @PolyDet("down")}$
%
...
We annotated the \codieid{equals()} method of the \<Set> class in the JDK as
\begin{verbatim}
...
\begin{figure}
    \todo{linebreak}
    $\infer[\sc{\mathit{SET \ PRECISION}}]{\Gamma \vdash \mathit{z : Det}}{\Gamma \vdash \mathit{x} : \mathit{OrderNonDet} \ \mathit{Set}\langle \kappa_{x} \ \beta_{x}\rangle, \mathit{y} : \mathit{OrderNonDet} \ \mathit{Set}\langle \kappa_{y} \ \beta_{y}\rangle & \mathit{z = x.equals(y)} & \langle \kappa_{x} \ \beta_{x}\rangle \nsqsubseteq \langle \mathit{OrderNonDet \ List} \rangle & \langle \kappa_{y} \ \beta_{y}\rangle \nsqsubseteq \langle \mathit{OrderNonDet \ List} \rangle }$
    
    \bigskip
    
    $\infer[\sc{\mathit{MAP \ PRECISION}}]{\Gamma \vdash \mathit{z : Det}}{\Gamma \vdash \mathit{x : OrderNonDet \ Map\langle Det \ \beta_k, \kappa_v\ \beta_v \rangle, y : Det \ \beta} & \mathit{z = x.get(y)}}$
