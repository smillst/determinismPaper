...
(Compare this to 
$\forallt{\kappa} \forallt{\beta} \kappa \ \beta \rightarrow \kappa \ \beta$
which differs because the qualifier and basetype can vary independently;
see below.)\todo{Does the foregoing make sense?}
...
\item
  ...
  Basetype polymorphism is rarely used.  Full type polymorphism is more
...
\OurTypeSystem uses the standard meaning of parametric
polymorphism~\cite{Abadi:1989:FIM:77350.77373,Plotkin:1993:LPP:645891.671433}.
A polymorphic abstraction (such as a class or function) is written and
typechecked once, but can be instantiated differently at different uses,
each of which behaves as if it was written with a different non-polymorphic
type.

Informally, the \<length> above acts as if it had multiple
...
The body must typecheck at every possible instantiation, and at a use site,
...
Consider this typing for the \<size> function:

$\<size>\ :\ \forallt{\kappa} \kappa\ \<Collection> \rightarrow \kappa\ \<Int>$

It cannot be instantiated as 

$\<size>\ :\ \<OrderNonDet>\ \<Collection> \rightarrow \<OrderNonDet> \<Int>$

because such an instantiation would include the invalid type \<OrderNonDet Int>.
A similar problem occurs for the \<first> operation that returns the first
element of a list.

\OurTypeSystem resolves this problem by introducing two variants to the
\<PolyDet> qualifier, \PolyDetUp\ and \PolyDetDown.
The $\uparrow$ operator converts \<OrderNonDet> to \<NonDet> and leaves the
other qualifiers unchanged.  The upward-pointing arrow is mnemonic of how
\<OrderNonDet> is changed:  it replaced by something higher in the type hierarchy.
...
$\<size> : \forallt{\kappa} \kappa\ \<Collection> \rightarrow \down{\kappa} \<Int>$

This can be instantiated at all three type qualifiers without creating any
invalid types, and the instantiations implement the semantics of \cref{sec:ond-behavior}:

$\<size> : \<NonDet>\ \<Collection> \rightarrow \<NonDet> \<Int>$

$\<size> : \<OrderNonDet>\ \<Collection> \rightarrow \<Det> \<Int>$

$\<size> : \<Det>\ \<Collection> \rightarrow \<Det> \<Int>$

An example use of $\uparrow$ is the \<first> operation.  A simplified
version of its type is

$\<first> : \forallt{\kappa\ \beta} \kappa\ \<List>\angles{\kappa\ \beta} \rightarrow \up{\kappa}\ \beta$

whose type qualifier can be instantiated as

$\<first> : \forallt{\beta} \<NonDet>\ \<List>\angles{\<NonDet>\ \beta} \rightarrow \<NonDet>\ \beta$

$\<first> : \forallt{\beta} \<OrderNonDet>\ \<List>\angles{\<OrderNonDet>\ \beta} \rightarrow \<NonDet>\ \beta$

$\<first> : \forallt{\beta} \<Det>\ \<List>\angles{\<Det>\ \beta} \rightarrow \<Det>\ \beta$

The actual type of \<first> accounts for more possible types for the type arguments.
...
$\<get> : \forallt{\kappa} \kappa\ \<List>\angles{\kappa\ \<String>} \times \kappa\ \<Int> \rightarrow \up{\kappa}\ \<String>$
...
$\<get> : \forallt{\kappa\ \beta} \<List>\angles{\kappa\ \beta} \times \kappa\ \<Int> \rightarrow \up{\kappa}\ \beta$
...
If either the list or the index is non-deterministic, then the result is
nondeterministic:
...
element of a list, because it would permit a deterministic collection must
not be side-effected in non-deterministic ways.  Consider the following
type for
