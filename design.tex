...
\begin{figure}
    $\infer[\rulename{any type}]{\wellformed{\kappa \  \beta}}{\kappa : \|Det / NonDet|}$
    
    \bigskip
    
    $\infer[\rulename{ordernondet collection}]{\wellformed{\kappa_c \  \beta_c \angles{\kappa_e \ \beta_e}}}{\kappa_c : \|OrderNonDet/Det| & \kappa_e \sqsubseteq \kappa_c & \beta_c \sqsubseteq \<Collection>}$
    
    \bigskip
    
    $\infer[\rulename{nondet collection}]{\wellformed{\|NonDet| \ \beta_c\angles{\|NonDet| \ \beta_e}}}{\beta_c \sqsubseteq \<Collection>}$
    
    \bigskip
    
%      $\infer[\rulename{ordernondet array}]{\wellformed{\kappa_e \ \beta_e \ \  \kappa_a []}}{\kappa_a : \|OrderNonDet/Det| & \kappa_e \sqsubseteq \kappa_a }$
%     
%     \bigskip
%     
...
%     \bigskip
    ...
    $\infer[\rulename{ordernondet map}]{\wellformed{\kappa_m \  \beta_m \angles{\kappa_k \ \beta_k, \kappa_v \ \beta_v}}}{\kappa_m: \|OrderNonDet/Det| & \kappa_k \sqsubseteq \kappa_m & \kappa_v \sqsubseteq \kappa_m & \beta_m \sqsubseteq \<Map>}$
    
    \bigskip
    
    $\infer[\rulename{nondet map}]{\wellformed{\|NonDet| \ \beta_m\angles{\|NonDet| \ \beta_k, \|NonDet| \ \beta_v}}}{\beta_m \sqsubseteq \<Map>}$
    
    \caption{Type well formedness}
\todo{The \rulename{any type} rule is not correct, because not every type is
...
As stated in the rule \rulename{any type}, the types \<Det> and \<NonDet>
...
\cref{type-validity}).
\Cref{fig:determinism-collections} restates its restrictions, and here are examples.
...
\smallskip
 ...
 Although such types could exist, they are
...
in its signature then the instantiation of \<PolyDet> at that location gets replaced by the appropriate type as shown in \cref{tab-poly-resolutions}.
...
\begin{figure}
    $\infer[\rulename{poly resolution}]{\pi_r \ \beta_r\ \<m>(\overline{\pi_i \ \beta_i})\{\ \}}{\forall \kappa \in \{\|NonDet, OrderNonDet, Det|\} : \kappa \ \beta_r\ \<m>(\overline{K \ \beta_i})\{\ \} & \pi_r, \pi_i \in \{\|PolyDet|, \|PolyDet|\uparrow, \|PolyDet|\downarrow    \}}$
...
\begin{figure}
    $\infer[\rulename{hashmap creation}]{\|new \ \kappa\ HashMap()|}{\kappa = \|OrderDet/NonDet|}$
    
    \bigskip
    
    $\infer[\rulename{treemap creation}]{\|new \ \kappa\ TreeMap()|}{\kappa = \|Det/NonDet|}$
    
    \bigskip
    
    $\infer[\rulename{hashset creation}]{\|new \ \kappa\ HashSet()|}{\kappa = \|OrderDet/NonDet|}$
    
    \bigskip
    
    $\infer[\rulename{treeset creation}]{\|new \ \kappa\ TreeSet()|}{\kappa = \|Det/NonDet|}$
