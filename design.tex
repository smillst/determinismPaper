...
\todo{This isn't true.  Using standard typing rules, the type of an array index
expression, "ni" in this example, doesn't have to be related to the array
element type. The Determinism Checker adds a rule for this.}
\todo{We also need to add a section about non-standard typing rules. We
implemented non-standard rules for both fields and arrays.}
\todo{ Also, we should point out that nondet collection types are not related
to order non det collection types.  But order non det collection types are a
supertype of det collection types.  We use this fact elsewhere in the paper,
...
polymorphism~\cite{AbadiPG1989,PlotkinA1993}.
...
\<OrderNonDet> collection would choose the \<NonDet> instantiation,
\todo{Because OrderNonDet collection is not a subtype of NonDet collections,
this isn't true.  It just would not be legal to call the method with an
OrderNonDet collection.  Any time @PolyDet is on both a collection type and a
non-collection type, then the method can't be called on an OrderNonDet
collection.  This is also true when @PolyDet("use") is used.}
and the
...
\todo{Because NonDet collections are not related to OrderNonDet collections, this is not the most specific instantiation. Without @PolyDet("up"), first can't be called on an OrderNonDet collection.}
...
\noindent
Then the invocation \<first(listOfLists)> is illegal. \todo{I don't understand why this is illegal, but \<first(myOndStrings)> isn't}
...
\noindent
would be instantiated at the least upper bound of the types of its first
and fourth arguments, and the third argument must conform to the
...
\begin{Verbatim}
    @PolyDet("up") E get(@PolyDet List<E> this, @PolyDeti("up") int index);
...
\todo{Add a section for @PolyDet("upDet") which should be renmaed to @PolyDet("shuffle")}
...
\todo{\<shuffle> should only be called on an @NonDet Collection or an
OrderNonDet collection that has no alaises that are Det collections.}
...
It might seem that the notion of a deterministic set or map is
...
\todo{The type argument must be Det for any base type.}
...
\todo{I don't understand why the return type isn't just PolyDet("up")?}
