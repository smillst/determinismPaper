...
In a type system,
a \textit{type} stands for a set of values.  It abstracts or restricts the
set of possbile run-time values that an expression may evaluate to.
...
A type checker can verify the correctness of this \textit{type} with its dataflow analysis and subtyping rules.
In Java, a type qualifier is written as a type annotation with a leading \<@>, as in
\<@Positive int>.
...
\begin{figure}
    ...
    $\infer[\textsc{method call}]{\mathit{method}(\overline{a_i}) : \tau_r}{\tau_r \  \mathit{method}(\overline{\tau_i \  p_i}) & \overline{a_i : \tau_i}}$
...
of \<OrderNonDet> with respect the the other two type qualifiers is \codeid{Det $\sqsubseteq$ OrderNonDet $\sqsubseteq$ NonDet} as shown in Figure~\ref{fig-determinism-hierarchy}.
...
\begin{figure}
    $\infer[\textsc{any type}]{\wellformed{\kappa \  \beta}}{\kappa : \mathit{Det / NonDet}}$
    
    \bigskip
    
    $\infer[\textsc{ordernondet collection}]{\wellformed{\kappa_c \  \beta_c \langle\kappa_e \ \beta_e\rangle}}{\kappa_c : \mathit{OrderNonDet/Det} & \kappa_e \sqsubseteq \kappa_c & \beta_c \sqsubseteq \mathit{Collection/Iterator}}$
    
    \bigskip
    
    $\infer[\textsc{nondet collection}]{\wellformed{\mathit{NonDet} \ \beta_c\langle\mathit{NonDet} \ \beta_e\rangle}}{\beta_c \sqsubseteq \mathit{Collection/Iterator}}$
    
    \bigskip
    
     $\infer[\textsc{ordernondet array}]{\wellformed{\kappa_e \ \beta_e \ \  \kappa_a []}}{\kappa_a : \mathit{OrderNonDet/Det} & \kappa_e \sqsubseteq \kappa_a }$
    
    \bigskip
    
    $\infer[\textsc{nondet array}]{\wellformed{\mathit{NonDet} \ \beta_e \ \  \mathit{NonDet} []}}{}$
    
    \bigskip
    
    $\infer[\textsc{ordernondet map}]{\wellformed{\kappa_m \  \beta_m \langle\kappa_k \ \beta_k, \kappa_v \ \beta_v\rangle}}{\kappa_m: \mathit{OrderNonDet/Det} & \kappa_k \sqsubseteq \kappa_m & \kappa_v \sqsubseteq \kappa_m & \beta_m \sqsubseteq \mathit{Map}}$
    
    \bigskip
    
    $\infer[\textsc{nondet map}]{\wellformed{\mathit{NonDet} \ \beta_m\langle\mathit{NonDet} \ \beta_k, \mathit{NonDet} \ \beta_v\rangle}}{\beta_m \sqsubseteq \mathit{Map}}$
...
the type of the type parameter (Rule $\textsc{ordernondet collection}$) (see Figure~\ref{fig-determinism-collections}).
...
elements are not allowed (Rules $\textsc{ordernondet array}$ and $\textsc{nondet array}$ in Figure~\ref{type-validity}).
...
\begin{figure}
    $\infer[\textsc{poly resolution}]{\pi_r \ \beta_r method(\overline{\pi_i \ \beta_i})\{\ \}}{\forall K \in \{\mathit{NonDet, OrderNonDet, Det\}} : K \ \beta_r method(\overline{K \ \beta_i})\{\ \} & \pi_r, \pi_i \in \{\mathit{PolyDet}, \mathit{PolyDet}\uparrow, \mathit{PolyDet}\downarrow    \}}$
...
the caller method \codeid{bar} to invoke it with both \<Det> and \<NonDet> arguments. The checker flags an error 
...
The determinism checker has additional type refinement rules for the sorting methods as shown in Figure~\ref{fig-sorting}. 
\begin{figure}
    $\infer[\textsc{list sort}]{\Gamma \vdash x : \mathit{Det \ List}\langle\mathit{Det} \ \beta\rangle}{\Gamma \vdash x : \mathit{OrderNonDet \ List}\langle\mathit{Det} \ \beta\rangle & \mathit{x.sort()}}$
    
    \bigskip
    
    $\infer[\textsc{collections sort}]{\Gamma \vdash x : \mathit{Det \ List}\langle\mathit{Det} \ \beta\rangle}{\Gamma \vdash x : \mathit{OrderNonDet \ List}\langle\mathit{Det} \ \beta\rangle & \mathit{Collections.sort.sort(x)}}$
    
    \bigskip
    
    $\infer[\textsc{arrays sort}]{\Gamma \vdash x : \mathit{Det \ \beta \ Det[]} }{\Gamma \vdash x : \mathit{Det \ \beta \ OrderNonDet[]} & \mathit{Arrays.sort( \ x)}}$
...
type argument of this \codeid{List} have the type annotation \codeid{Det} (Rule LIST SORT in Figure~\ref{fig-sorting}). Similar rules apply for \codeid{Arrays.sort()} and \codeid{Collections.sort()}.
...
\begin{figure}
    $\infer[\textsc{hashmap creation}]{\mathit{new \ \kappa\ HashMap()}}{\kappa = \mathit{OrderDet/NonDet}}$
    
    \bigskip
    
    $\infer[\textsc{treemap creation}]{\mathit{new \ \kappa\ TreeMap()}}{\kappa = \mathit{Det/NonDet}}$
    
    \bigskip
    
    $\infer[\textsc{hashset creation}]{\mathit{new \ \kappa\ HashSet()}}{\kappa = \mathit{OrderDet/NonDet}}$
    
    \bigskip
    
    $\infer[\textsc{treeset creation}]{\mathit{new \ \kappa\ TreeSet()}}{\kappa = \mathit{Det/NonDet}}$
...
\todo{Talk about this design choice. Josh Blochs book: "If the code surrounding the first declaration depended on LinkedHashSet's ordering policy,
...
\end{verbatim}
While this is sound, it is imprecise in the case of \codeid{OrderNonDet Sets}. We added the following rule shown in Figure~\ref{fig-precision-rules} to our checker to improve precision.
\begin{figure}
    \todo{linebreak}
    $\infer[\textsc{set precision}]{\Gamma \vdash \mathit{z : Det}}{\Gamma \vdash \mathit{x} : \mathit{OrderNonDet} \ \mathit{Set}\langle \kappa_{x} \ \beta_{x}\rangle, \mathit{y} : \mathit{OrderNonDet} \ \mathit{Set}\langle \kappa_{y} \ \beta_{y}\rangle & \mathit{z = x.equals(y)} & \langle \kappa_{x} \ \beta_{x}\rangle \nsqsubseteq \langle \mathit{OrderNonDet \ List} \rangle & \langle \kappa_{y} \ \beta_{y}\rangle \nsqsubseteq \langle \mathit{OrderNonDet \ List} \rangle }$
    
    \bigskip
    
    $\infer[\textsc{map precision}]{\Gamma \vdash \mathit{z : Det}}{\Gamma \vdash \mathit{x : OrderNonDet \ Map\langle Det \ \beta_k, \kappa_v\ \beta_v \rangle, y : Det \ \beta} & \mathit{z = x.get(y)}}$
...
receiver or the argument happens to be an \codeid{OrderNonDet List} (An order nondeterminstic list does not provide any guarantee on the
equality as it is dependent on the iteration order). 

Similarly, the \codeid{get()} method in the \<Map> class is annotated with polymorphic annotations and the rule $\sc{\mathit{MAP \ PRECISION}}$ in Figure~\ref{fig-precision-rules} improves precision for \<OrderNonDet> receivers.
...
\subsection{Extending the determinism checker}\label{extending-det-checker}
