...
all the case study materials are publicly available at \myurl{\todo{url for
    case study}} for reproducibility. \todo{This should include our
...
\todo{In some cases, the number of \<@Det> annotations was especially prevalent.
  In these cases we changed the default qualifier for that class to \<@Det>.}
...
Refactoring can also eliminate the last warning. \todo{Can it? I thought
  what happens when iterating over an @OrderNonDet collection is something we
can't verify with the checker.}
