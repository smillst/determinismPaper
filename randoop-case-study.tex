...
%      2 iterating over @OrderNonDet collection to modify another
  ...
  \<@NonDet>.  For example, the following code is safe,
  \theDeterminismChecker issues a warning.  Variable \<elt> has type
  \<@NonDet>, so the call to \<add> does not typecheck.
\begin{Verbatim}
...
local refactoring in the Randoop codebase can make the code typecheck.
