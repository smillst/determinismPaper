...
\Cref{code-determinism} shows that the determinism qualifier on the return type of \<entrySet()> is the
same as that on the receiver.  That is, its type (ignoring type arguments) is
...
differently with respect to determinacy:  either all, or none, or deterministic.
