...
We have implemented \ourTypeSystem for Java, in a tool named \theDeterminismChecker.
...
\Cref{fig-creation-rules} formalizes the type wellformedness rules for maps and sets. Similar constraints apply to \<HashSet>s and \<TreeSet>s which are
...
\begin{figure}
    ...
    \todo{Are these type well-formedness rules?}
    \todo{What about \<new TreeSet(myOrderedCollection<Det ...>)> which yields a
        deterministic set?}
